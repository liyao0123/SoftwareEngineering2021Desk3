...
#### 1. Designs and Techniques
 ...
 - **Database:**    
   ...
   In our situation of short-time development of web pages, this help is precisely what we need.
   <br/>   
   Everything has two sides. The limitation of using MySQL is that it seems more complicated than NoSQL technology, especially in a small project. 
   We need to configure connection information and manage dependencies. After that, we need to build the database, create tables, insert values and declare keys.
   Luckily, MyBatisPlus could simplify the query work. Without that tool, I could not imagine how many tedious things we must do.<br/>
   <br/>
 - **Back End:**    
   ...
   Above all, SpringBoot could help us start a web project quickly and doesn't require any configuration files. By the way, the embedded server tomcat is so convenient that our web project could run and test on a server directly.
   With that, we could test the request and response in advance.
   Besides, introducing the dependency devtools could achieve hot deployment. That means, when we make some changes on the HTML file, then press `CTRL+F9` and refresh the page, the new contents would show up. It is very time-saving.
   In addition, another useful developing tool lombok would eliminate the declaration of methods and constructors in a class. With `@Data` annotation, everything is done.<br/>      
   It is evident that disadvantages and advantages correspond to each other. Even though developers could start a web application with SpringBoot quickly, they must study more to do deep customization. To understand how the SpringBoot works, understanding JavaWeb, Spring, SpringMVC and MyBatis is necessary.
   This requires a certain accumulation of knowledge.  
   <br/>
 - **Front End:**    
   Based on HTML, CSS, JavaScript, we make use of Thymeleaf and Bootstrap. SpringBoot recommends Thymeleaf to work with when developing a web application. This modern server-side Java template engine could bring elegant natural templates to our development workflow.
   "It means that HTML can be correctly displayed in browsers and also work as static prototypes, allowing for stronger collaboration in development teams. With modules for Spring Framework, a host of integrations with your favourite tools, and the ability to plug in your functionality, Thymeleaf is ideal for modern-day HTML5 JVM web development."[3]
   Since we have already developed with Spring frames, it is better to work with Thymeleaf together.   
   <br/>
   As for Bootstrap, Bootstrap is a popular front-end framework that streamlines website design. In addition, it allows for the creation of accessible and responsive web layouts[4].
   With so many valuable and good-looking elements, developers could build a web page in a short time. Moreover, it is so friendly for our team that we don't have to achieve the function by typing each code but introduce one class we need. With bootstrap, we could have more time considering the process and logic.
   <br/>   
   Tools are tools, after all. They are designed for general needs. When we want some unique design, we still have to get back to the original syntax of JavaScript and do some customization. What's worse, applying Thymeleaf grammars to each HTML tags seems not that easy. In some situations, we have to give it up and get back to the HTML itself, even if there is much more it can do. 
#### 2. Comparison with MEAN
There are three reasons why we chose other stacks rather than MEAN. 

The first and the most primary factor is that we had the foundation of Spring frames. In the beginning, our team planned to learn about the MEAN stack. Two members would study front-end knowledge, and others would study the rest. As time goes on, everyone tried the best to keep up the pace. However, the fact that we have no more time to learn new technology and apply it to our project before the deadline pushed us to make a change. Haojie Xin would be responsible for almost all development work because he has the foundation of Spring frames. Others keep learning and do some help when Haojie needs help. Thanks for the Easter vacation. During the vacation, we did the final sprints and prepared the knowledge that we need. One of them is SpringBoot, which is embedded with the tomcat server and offers web development starters. With this comprehensive frame, we don't have to learn the Express frame and Node.js, respectively.

The second reason is that we want to deepen the understanding and application of MySQL. In the tools, we met the MariaDB for the first time and did some exercises about it. The contents are not enough for us to understand the database totally. Therefore, we decided to use MySQL in our project so that we could apply it in the actual project. After the whole process, it did encourage us to learn more about MySQL. We command how to design and build a database according to our own needs and users' needs. This helpful experience would definitely benefit our future work. Compared with MongoDB, these two databases have their strengths and weaknesses. From the view of the data we used in our project, these two are both suitable. However, as we said before, we have no more time to learn and get familiar with new technology, especially there is only one student who could do the development job. Therefore, to use what we have learned seems to be a sensible choice. If we have spare time after the deadline, it would be an excellent chance to pick up the knowledge of MongoDB.

Last but not least, the main languages of the front-end API we use are HTML, CSS and JavaScript. We need to do some customization according to users' needs with the above languages. If we apply Angular, we need to study the frame first and then integrate it with the API. Changing the original code with a different language is not a good try. There may be many problems and bugs we need to fix. Therefore, there is no need to use Angular in our project. However, it is undoubtedly a tool we will try in the future. With the frame, the developing work would be easier and happier.
